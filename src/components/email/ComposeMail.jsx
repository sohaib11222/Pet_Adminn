...
const ComposeMail = () => {
  ...
  const loadFile = async (event) => {
    ...
    setAttachmentIsImage(Boolean(file.type && file.type.startsWith("image/")));
    try {
      const formData = new FormData();
      formData.append("file", file);
      const res = await apiRequest("/upload/chat", {
        method: "POST",
        body: formData,
        timeoutMs: 60000,
      });
          ...
          <div className="row">
            ...
            <div className="col-12 col-md-6 col-xl-12">
              <div className="form-group local-top-form">
                ...
                {attachmentError ? (
                  <div className="text-danger mt-1">{attachmentError}</div>
                ) : null}
                {attachmentUrl ? (
                  attachmentIsImage ? (
                    <div className="upload-images upload-size mt-2">
                      <img src={toPublicUrl(attachmentUrl)} alt="Attachment" />
                    </div>
                  ) : (
                    <div className="mt-2">
                      <a href={toPublicUrl(attachmentUrl)} target="_blank" rel="noreferrer">
                        {attachmentUrl}
                      </a>
                    </div>
                  )
